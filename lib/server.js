...
class Server {
	 ...
	 */
	decideFocus(ns) {
		// Home server always uses attk.
		if ("home" === this.hostname) {
			this.focus = canRunAttk() ? "attk" : "";
			return;
		}

		// Purchased servers can only be used for attk (cannot hack themselves).
		if (this.purchasedByPlayer) {
			this.focus = "attk";
			return;
		}

		// When home cannot coordinate attk.js, then we focus on sgrw.js (early game).
		if (!canRunAttk()) {
			this.focus = "sgrw";
			return;
		}
