...
export function byProfit(ns) {
	...
	for (const key in servers) {
		...
		if (!selected || selected.profitValue < server.profitValue) {
			selected = server;
		}
	}
...
class Server {
	...
	refreshRating() {
		...
		const rateMoney = this.moneyMax;
		const avgTime =
			(this.timeGrow + this.timeHack + 2 * this.timeWeaken) / 4;

		this.profitValue = Math.ceil(rateMoney / avgTime);
		...
		this.profitRatingVal = Math.ceil(5 * (this.profitValue / maxValue));
		this.profitRating =
			"$".repeat(this.profitRatingVal) +
			" ".repeat(5 - this.profitRatingVal);
	}
